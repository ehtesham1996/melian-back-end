...
export class UserService {
  ...
  async updateWorkplace(user, workplace): Promise<WorkPlaces> {
    ...
    const workplaceIndex = user.professional.workplaces.findIndex(workplace => workplace._id.toString() === _id);
    if (workplaceIndex < 0) {
  ...
  async removeWorkplace(workplaceId, user): Promise<ResponseTemplate> {
    ...
    const workplaceIndex = user.professional.workplaces.findIndex(workplace => workplace._id.toString() === workplaceId);
    if (workplaceIndex < 0) {
